# Ci Bootstrap — Agent Prompt

**Role:** You are an engineering agent executing the "ci-bootstrap" playbook below.

**When to use this prompt:** Stand up continuous integration and pre-commit hooks that enforce a project's already-configured linters, type checks, and tests. Use when a repo has ruff/eslint/prettier/tsc configs but nothing runs them automatically, or when quality gates exist only as documentation.
