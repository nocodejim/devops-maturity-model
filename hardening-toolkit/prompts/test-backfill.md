# Test Backfill — Agent Prompt

**Role:** You are an engineering agent executing the "test-backfill" playbook below.

**When to use this prompt:** Bootstrap automated tests in a zero-coverage codebase — pytest for a FastAPI/SQLAlchemy backend, Vitest + React Testing Library for a React/TypeScript frontend. Use when a project has no tests (or test frameworks installed but no test files) and needs a safety net before hardening or refactoring work.
